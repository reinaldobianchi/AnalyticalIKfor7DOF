...
# Type 4: Eating
traj_type4 = [
    [0.20, 0.00, 0.10],  # 0: table plate
    [0.15, 0.10, 0.15],  # 1: grab food
    [0.10, 0.18, 0.22],  # 2: lift
    [0.00, 0.20, 0.30],  # 3: MOUTH ✓
    [0.10, 0.18, 0.22],  # 4: leave mouth
    [0.15, 0.10, 0.15],  # 5: lower
    [0.20, 0.00, 0.10]   # 6: plate
]

phi_s_vals = [52, 42, 38, 30, 38, 42, 52]  # pre-calculated

print("=== TYPE 4: EATING ===")
print(f"{'#':<2} {'Pose':<20} {'ϕ_s':<4}")
print("-"*28)

for i, (pose, phi) in enumerate(zip(traj_type4, phi_s_vals)):
...
ax1.set_xticklabels(['Table','Grab','Lift','Mouth','Leave mouth','Lower','Table'])
ax1.set_ylabel('ϕ_s (°)')
ax1.set_title('Type 4: ϕ_s ~30° in the mouth')
ax1.grid(True)
ax1.legend()
plt.tight_layout()
plt.savefig('phi_s_evolution_type4.png')

# IMAGE 2: Trajectory XY
fig2, ax2 = plt.subplots(figsize=(6, 4))
x_vals = [p[0] for p in traj_type4]
y_vals = [p[1] for p in traj_type4]
ax2.plot(x_vals, y_vals, 'ro-', linewidth=3, markersize=10)
ax2.scatter(0.00, 0.20, c='red', s=150, label='Mouth')
ax2.scatter(0.20, 0.00, c='green', s=100, label='Plate')
ax2.set_xlabel('X')
ax2.set_ylabel('Y')
ax2.set_title('Trajectory XY Type 4')
ax2.legend()
ax2.grid(True)
ax2.axis('equal')
plt.tight_layout()
plt.savefig('trajectory_xy_type4.png')
