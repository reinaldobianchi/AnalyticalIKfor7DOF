...
#     return np.vstack([z1, z2, z3]).T                                # Stacks the vectors z1, z2, z3 as a column. Result: a 3×3 matrix
...
def compute_jacobian(q):
    ...
    for i in range(7):
        dq = np.copy(q)                                              # Creates a copy of q → dq (reserving original q)
...
traj_type3 = np.array([
    [0.00,  0.0,  0.10],  # rest
    [0.15, -0.20, 0.10], # halfway
    [0.30, -0.40, 0.10], # ping-pong target
    [0.15, -0.20, 0.10], # return
    [0.00,  0.00, 0.10]   # rest.
...
# Single plot (without subplots and specific colors)
plt.figure()
# plt.plot(psi_h_list)
# plt.plot(psi_m_list)
...
plt.xlabel("Point in the trajectory (Type 3)")
plt.ylabel("angle ψ (rad)")
plt.title("Comparison: ψ human vs ψ manipulability")
plt.legend()
plt.show()

# Single plot (without specific colors and subplots)
plt.figure()
plt.plot(delta_psi)
plt.xlabel("Point in the trajectory (Type 3)")
plt.ylabel("Δψ (rad)")
